...
class DetailsSection
{
  ...
  getTitleComponents(title) {
    return title.toLocaleLowerCase().split(" ").filter(word => {
  ...
  render(integrations, step) {
    ...
    const similarSteps = Object.values(integrations.steps).map(step2 => {
      return {
        step: step2, 
        categoryScore: step2.categories.filter(category => step.categories.indexOf(category) > -1).length,
        titleScore: this.getTitleComponents(step2.title).filter(category => this.getTitleComponents(step.title).indexOf(category) > -1).length,
      };
    }).filter(scoredStep => {
    ...
    }).map(scoredStep => scoredStep.step).slice(0, 4);
